...
14. **Scale by physical card identity and render only active-filter rows**: when an account can contain many observations or repeated product names, maintain one account-wide coverage projection by physical card and include product plus explicit ending digits in every rendered group label. Coverage metrics include benefit-bearing, conclusively empty, unresolved latest-scan, and older retained records, but the card list is filter-aware: render a group only when it has at least one row in the selected filter. `Remaining` is the default and contains every non-`Used` row; `Used` contains only `Used` rows; membership never relabels a row. An all-used card is absent from `Remaining` and appears with rows under `Used`; zero-benefit partial/failed/stale/older-retained records create no empty card shell, while their quality remains in account-level counts and data notes. Partial/stale cards that do have rows retain their card-level quality badge and disclosure. Remove compact zero-row groups and use one filter-specific account empty state that points to the other filter when applicable. Reconcile attempted latest-scan cards, total stored records, confirmed-empty records, unresolved records, and older retained records. Show conclusive account-level no-trackable-benefits copy only when every stored card is confirmed empty and the latest scan itself is complete. A site-wide reader may keep collapse/filter state only in panel memory, but scanning and cancelling must force the full progress/cancellation workspace to remain reachable.
...
| Duplicate primary products or a high-observation account are restored | Keep physical cards distinct by product plus ending digits, but render a group only when it has rows in the active filter; preserve all coverage and quality counts account-wide |
| Stored-card count exceeds latest attempted-card count | Reconcile attempted, stored, unresolved, confirmed-empty, and older-retained counts explicitly; omit zero-row older records from the card list without losing their data-note count |
| Latest-scan card has zero benefits after a partial catalog HTTP failure | Omit its empty card shell, keep it unresolved in account-level quality counts/copy, and never claim the partial read proved no trackable benefits |
| Benefit-bearing card has only Used rows while `Remaining` is selected | Render no card group for it; keep Used count available and render the card with rows and quality disclosure after selecting `Used` |
| No card has a row in the active filter | Render one filter-specific account empty state; direct the user to the other filter when it contains rows |
...
- **Presentation good**: a partial card with an active-filter row keeps one accessible `Partial data` badge and card-level details; duplicate primary products remain distinct by ending; unresolved/stale zero-benefit records remain in account-level counts but create no empty group; and `Remaining` contains exact non-used labels without flattening them.
- **Presentation base**: an all-used card is absent under `Remaining`, while the account Used count stays available and selecting `Used` renders that card's used rows and quality disclosure. A catalog failure with no rows contributes to unresolved/Data notes rather than a card shell. A valid V2 quarterly source range renders as `Jul–Sep 2026` rather than `QuarterYear`.
- **Presentation bad**: retain compact `0 remaining benefits` groups, render unresolved zero-row card shells, replace benefit state with card quality, group duplicate products only by name, mix stored/attempted counts without retained-card context, show raw `CalenderYear` despite a structured range, or relabel every Remaining row as not used.
...
- all six truthful benefit labels follow the required precedence, observation-quality labels remain independent and appear once per rendered accessible card heading rather than on each row, `Remaining`/`Used` state and counts are accessible, only groups with rows in the active filter render, all-used cards move from absent under Remaining to row-bearing under Used, unresolved/stale/older-retained zero-benefit records create no card shells but remain in account-level quality counts, attempted/stored/confirmed/unresolved/retained totals reconcile, conclusive no-benefits copy requires a complete latest scan, and duplicate primary products remain distinct by ending digits;
...
```ts
// Wrong: parser quality overwrites the benefit state, Remaining flattens
// every non-used row, and the card shell remains even when no row matches.
...
// Correct: derive independent facts. Filter membership controls both row and
// group presence, while account coverage retains omitted quality counts.
...
This separation prevents data-collection uncertainty from becoming a benefit state and prevents diagnostic-only or zero-count card shells from occupying the active benefit list.
...
- deterministic partial/failure paths exercise the built artifact and exact retry/error behavior; unresolved/stale zero-benefit records affect aggregate quality copy without producing card groups, and all-used groups move from absent under Remaining to visible with rows under Used;
