...
21. **Historical replay is manifest-scoped.** APPLIED replay and rollback use the original manifest/evidence authority for manifest-covered units and tolerate unrelated later inventory changes. Blocked, unmanifested rows on the same database page neither gain authority nor invalidate that historical replay. A manifest-covered unit that becomes blocked by attachment/provenance/graph drift remains on the reviewed page and is returned as an aggregate closed stop by `rollback-preview`; manifest validation must require every manifest key to remain present and every currently safe proposal to remain manifest-authorized, but must not reject the page solely because one authorized entry is now blocked.
...
24. **Blocked parity is own-state parity, not naive whole-card equality.** A blocked or unmanifested definition may share a physical card with a safely applied sibling. Final parity reconstructs the original reviewed page slices from the complete pre-apply proposals, page fingerprints, and cursor chain, then reverses only exact `APPLIED_VALID` sibling effects covered by the complete manifest bundle before comparing the blocked definition. The blocked source/status/audit/provenance/evidence graph must still equal its baseline exactly.
25. **Sibling normalization is closed multi-page authority.** Same-card repairs may cross bounded pages. Each reversed sibling must match one reviewed manifest fingerprint and entry fingerprint from the complete ordered bundle, plus exact inventory/evidence/postimage authority. Invalid, unmanifested, wrong-page, ambiguous-boundary, or drifted sibling evidence is a parity stop; it never authorizes a relaxed fingerprint comparison.
...
| A blocked unit's wider card graph changed only through exact manifest-covered `APPLIED_VALID` siblings | Reconstruct reviewed page authority, reverse those exact sibling effects for comparison, and require the blocked unit's own graph to remain identical |
| Same-card sibling evidence is invalid, unmanifested, belongs to no unique reviewed page, or the blocked unit itself changed | `blocked_unit_changed` / manifest-authority stop; do not relax fingerprints or mark protected state true |
