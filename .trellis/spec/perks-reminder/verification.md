...
14. Complete parity reads use one explicit bounded repeatable-read transaction, independent of the shorter shared request-path default. Tests assert the exact parity max-wait/timeout and that recognizable expiry is a fixed safe timeout while unrelated native errors remain the fixed generic database error.
15. Post-read gate failures must remain diagnosable without private data. The verifier carries/prints only the closed aggregate report and exits nonzero. Blocked-unit parity reconstructs reviewed page boundaries and reverses only exact same-card manifest-covered sibling effects; same-page, mixed-page, invalid-evidence, and true blocked-drift regressions are required.
...
| Complete parity read exceeds its explicit timeout | Return the fixed safe parity-timeout classification; emit no partial success report |
| Parity comparison gates fail after a successful read | Emit only the aggregate boolean/count/action/stop report and exit nonzero |
| Blocked whole-card fingerprint changed because a reviewed sibling applied | Normalize only exact manifest-covered sibling effects, then compare the blocked unit's own graph; fail closed on any authority or own-state drift |
...
For schema/spec work, assert the additive migration invariant suite passes; schema parses with no generated-file diff; canonical global target relations are restrictive while user-owned source/ledger/owner/card/parent/keeper evidence relations cascade; all five category-repair scenarios expose ordered seven-section headings; Markdown links/paths and structured task/config files parse; no sensitive/private evidence appears; parent confirmation/cleanup hold is present; package context remains discoverable; and `git diff --check` passes. For implementation completion, additionally run the full Jest suite, strict TypeScript, changed-source lint, affected public-DB/card-template/userscript invariants, task JSON/JSONL parsing, static sensitive/artifact review, and complete-diff review. Focused tests must cover all planner/persistence/runtime/AMEX/clone contracts, bounded cron loading, semantic evidence order, portable fingerprints, SQL-null clone preimages, lifecycle/legacy utility guards, malformed-evidence fail-closed behavior, explicit parity transaction bounds/error classification, aggregate failed-gate output, reviewed-page reconstruction, same-page/cross-page sibling normalization, and true blocked-unit drift. Database and production checks remain separately authorized and truthfully skipped here.
