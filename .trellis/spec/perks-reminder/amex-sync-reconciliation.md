...
This is the current contract for production userscript `1.0.1` and `amex-sync-envelope/3`; envelope V3 began during development at userscript `0.5.0`, public release `1.0.0` was the prior installed artifact, and the Quiet Ledger reader update increases that version strictly. It supersedes the historical Platinum-only envelope-V2 mapping contract in [Browser-Side Authenticated Read Integrations](browser-read-integrations.md). The local reader remains product-independent; destination identity is introduced only at the synchronization boundary.
