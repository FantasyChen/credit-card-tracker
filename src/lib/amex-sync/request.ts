...
import { resolveApprovedAmexSyncApplicationOrigin } from "@/lib/amex-benefit-reader/handoff-target";
import { parseAmexSyncEnvelope } from "@/lib/amex-benefit-reader/sync-contract";

export const AMEX_SYNC_REQUEST_MAX_BYTES = 300 * 1024;

const previewRequestShape = z.object({ envelope: z.unknown() }).strict();
...
export function assertSameOriginAmexSyncRequest(request: Request): void {
  const expectedOrigin = resolveApprovedAmexSyncApplicationOrigin(PRIMARY_SITE_URL);
  if (!expectedOrigin
  ...
  if (!/^application\/json(?:\s*;.*)?$/i.test(contentType)) throw new AmexSyncRequestError("content_type_invalid");
...
async function parseBody(request: Request): Promise<unknown> {
  ...
  try { return JSON.parse(text) as unknown; } catch { throw new AmexSyncRequestError("request_invalid"); }
}
...
export async function parsePreviewRequest(request: Request): Promise<ParsedPreviewRequest> {
  ...
  try { return { envelope: parseAmexSyncEnvelope(parsed.data.envelope) }; }
  catch { throw new AmexSyncRequestError("request_invalid"); }
}
...
export async function parseConfirmRequest(request: Request): Promise<ParsedConfirmRequest> {
  ...
  if (!parsed.success) throw new AmexSyncRequestError("request_invalid");
  try {
    return { envelope: parseAmexSyncEnvelope(parsed.data.envelope), proposalToken: parsed.data.proposalToken };
  } catch { throw new AmexSyncRequestError("request_invalid"); }
}
